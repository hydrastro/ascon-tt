...
//
// Phase 2.3 Ascon-AEAD128 encryption core with associated-data support.
//
// Scope of this module:
//   - Encryption only.
//   - Associated data supported.
//   - Arbitrary associated-data and plaintext lengths in bytes.
...
//   msg_bytes_i declares total plaintext length. The core requests
//   ceil(msg_bytes_i / 16) plaintext blocks on plaintext_*.
//   ciphertext_bytes_o reports the valid byte count for each ciphertext block.

module ascon_aead128_enc_ad #(
  ...
  localparam [3:0] ST_DRAIN_CT          = 4'd10;
  localparam [3:0] ST_FINAL_WAIT        = 4'd11;
  localparam [3:0] ST_TAG_WAIT          = 4'd12;
  ...
  wire [63:0] msg_part_m0_w    = m0_w & msg_part0_mask_w;
  wire [63:0] msg_part_m1_w    = m1_w & msg_part1_mask_w;
  wire [63:0] msg_part_c0_w    = (s0_w ^ msg_part_m0_w) & msg_part0_mask_w;
  wire [63:0] msg_part_c1_w    = (s1_w ^ msg_part_m1_w) & msg_part1_mask_w;
  wire [63:0] msg_part_s0_w    = (msg_final_bytes_q < 5'd8) ?
                                 (s0_w ^ msg_part_m0_w ^ pad64(msg_final_bytes_q)) :
                                 (s0_w ^ msg_part_m0_w);
  wire [63:0] msg_part_s1_w    = (msg_final_bytes_q < 5'd8) ?
                                 s1_w :
                                 ((msg_final_bytes_q == 5'd8) ?
                                  (s1_w ^ ASCON_PAD0) :
                                  (s1_w ^ msg_part_m1_w ^ pad64(msg_part1_bytes_w)));
    ...
    if (!rst_n) begin
      ...
      case (state_q)
        ...
        ST_MSG_WAIT_PARTIAL: begin
          if (plaintext_valid_i && plaintext_ready_o) begin
            ciphertext_block_o   <= {msg_part_c0_w, msg_part_c1_w};
            ciphertext_bytes_o   <= msg_final_bytes_q;
            ciphertext_valid_o   <= 1'b1;
